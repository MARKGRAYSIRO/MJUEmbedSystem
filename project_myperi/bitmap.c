...
int bmp_read (char *fn)
{
    ...
    if( ioctl(fbfd, FBIOGET_VSCREENINFO, &fbvar) ) // get screen info
    {
    ...
    if (fbvar.bits_per_pixel != 32) // if !=per pixel isnt 4byte we cant use it
    {
        fprintf(stderr, "bpp is not 32\n");
        exit(1);
    }

    screen_width    =   fbvar.xres; // x 720
    screen_height   =   fbvar.yres; // y 480
    bits_per_pixel  =   fbvar.bits_per_pixel; // 4
    line_length     =   fbfix.line_length;
    mem_size    =   line_length * screen_height; // 720 * 480 * 4
    ...
    pfbmap/*this is the char pointer of memory buffer*/ =   (unsigned char *)
    ...
    // fb clear - black (twise of for loop)x and y
    for(coor_y = 0; coor_y < screen_height; coor_y++) {
        ...
        {
            *ptr++  =   0x000000; 
            /*    *ptr++ = 0x000000 is same with 
             * pfbmap[ (coor_y*screen_width + coor_x)*4+0 ] = 0
             * pfbmap[ (coor_y*screen_width + coor_x)*4+1 ] = 0
             * pfbmap[ (coor_y*screen_width + coor_x)*4+2 ] = 0
             * pfbmap[ (coor_y*screen_width + coor_x)*4+3 ] = 0
             * because its 4byte that why i used +0 +1 +2 +3  
             * and its same with
             * ((unsigned long*)(pfbmap))[ (coor_y*screen_width + coor_x) ] = 0x0
             * because its int shape pointer(4byte)*/            
        }
    }

// direction for image generating : (0,0)-> (1,0)-> (2,0)-> ...-> (row, column) from bit map file !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
int bmp_read2 (char *fn)
{
    ...
    if( (fbfd = open(FBDEV_FILE2, O_RDWR)) < 0)
    {
        printf("%s: open error\n", FBDEV_FILE2);
        exit(1);
    }
